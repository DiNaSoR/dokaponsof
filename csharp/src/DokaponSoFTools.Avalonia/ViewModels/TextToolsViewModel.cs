...
public sealed partial class TextToolsViewModel : ObservableObject, IGamePathAware
{
    ...
    [ObservableProperty][NotifyPropertyChangedFor(nameof(CurrentEntries))] private int _selectedTabIndex;
    [ObservableProperty] private string _statsText = "";

    /// <summary>The entry collection for the active category tab.</summary>
    public ObservableCollection<DisplayTextEntry> CurrentEntries => SelectedTabIndex switch
    {
        1 => DialogEntries,
        2 => LabelEntries,
        3 => HudEntries,
    ...
    [RelayCommand]
    private async Task ExportTextsAsync()
    {
        if (_rawEntries.Count == 0) { Log.Warning("No texts loaded"); return; }

        string? path = await StorageService.SaveFileAsync("Save Texts", "Text files|*.txt|CSV|*.csv|JSON|*.json", "texts.txt");
        if (path is null) return;

        if (path.EndsWith(".json", StringComparison.OrdinalIgnoreCase)) { ExportAsJson(path); return; }
        if (path.EndsWith(".csv", StringComparison.OrdinalIgnoreCase)) { ExportAsCsv(path); return; }
    ...
    private async Task ImportTextsAsync()
    {
        ...
        string? textsPath = await StorageService.OpenFileAsync("Select Modified Texts", "Text files|*.txt");
        if (textsPath is null) return;
        string? offsetsPath = await StorageService.OpenFileAsync("Select Offsets File", "Text files|*.txt");
        if (offsetsPath is null) return;
        string? outputPath = await StorageService.SaveFileAsync("Save Modified Executable", "Executable|*.exe", "modded.exe");
