...
class LZ77Decompressor:
    ...
    def decompress_data(self, data: bytes) -> Optional[bytes]:
        """Decompress LZ77 data directly from bytes."""
        try:
            # Parse header from data
            if len(data) < 16:
                self.logger.error("Data too short for header")
                return None
            ...
            header = LZ77Header(magic, size, flag1, flag2)
            self.logger.debug(f"Header: {header}")
            
            # Initialize output buffer
            output = bytearray()
            
            # Process data (skip 16 byte header)
            compressed_data = data[16:]
            
            final_size = self.process_data_with_markers(compressed_data, output, header.flag1, header.size)
