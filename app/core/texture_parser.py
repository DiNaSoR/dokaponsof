...
from .cell_parser import CellChunk
...
def _decompress_lz77_cell(storage: bytes) -> tuple[bytes, LZ77Info | None]:
    """Decompress LZ77 data using the cell variant via the unified lz77 module."""
    result = _decompress_lz77(storage, variant="cell")
    # The cell variant returns (bytes, CellLZ77Info | None)
    return result
...
def parse_texture_parts_payload(payload: bytes) -> TexturePartsContainer:
    ...
    if storage.startswith(PNG_SIGNATURE):
        ...
        atlas_bytes, lz77 = _decompress_lz77_cell(storage)
    else:
